...
def solve_pde(img):
    ...
    # Create the nonlinear SNES problem
    problem = NonlinearPDE_SNESProblem(F, up, isothermal_bc)
    ...
    u_initial = lambda x: (
        -1000 * np.ones_like(x[0]),
        -1000 * np.ones_like(x[0]),
    )  # Example initial condition for u
    p_initial = lambda x: 20 * np.ones_like(x[0])  # Example initial condition for p
    ...
    # Apply the changes back to the mix
    b = dolfinx.la.create_petsc_vector(W.dofmap.index_map, W.dofmap.index_map_bs)
    ...
    snes.setTolerances(rtol=1.0e-6, max_it=50)
    snes.getKSP().setType("cg")
    snes.getKSP().setTolerances(rtol=1.0e-6)
    snes.getKSP().getPC().setType("lu")
    snes.getKSP().getPC().setFactorSolverType("umfpack")

    log.set_log_level(log.LogLevel.INFO)
    snes.solve(None, up.vector)
    # assert snes.getConvergedReason() > 0
    # assert snes.getIterationNumber() < 6

    print("Iteration number: ", snes.getIterationNumber())
    print("Function norm: ", snes.getFunctionNorm())
